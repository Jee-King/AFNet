...
from ltr.dataset import EOTB,Got10k
...
def run(settings):
    ...
    # Train datasets
    # got10k = Got10k(settings.env.got10k_dir, split='train')
    eotb_train = EOTB(settings.env.eotb_dir,split='train')

    # Validation datasets
    eotb_test = EOTB(settings.env.eotb_dir,split='val')
    # got10k_test = Got10k(settings.env.got10k_dir, split='votval')
    ...
    dataset_train = sampler.ATOMSampler([eotb_train], [1],
    ...
    dataset_val = sampler.ATOMSampler([eotb_test], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,
    ...
    trainer.train(20, load_latest=True, fail_safe=True)
