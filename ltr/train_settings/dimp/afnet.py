...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq,EOTB
...
def run(settings):
    ...
    dataset_val = sampler.DiMPSampler([eotb_test], [1], samples_per_epoch=5000, max_gap=200,
                                      num_test_frames=3, num_train_frames=3,
                                      processing=data_processing_val)

    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=51, stack_dim=1)
    ...
    optimizer = optim.Adam([{'params': actor.net.classifier.parameters(), 'lr': 1e-3},
                            ...
                            {'params': actor.net.align_fusion.parameters()}],
                           lr=2e-4)
    ...
    trainer.train(52, load_latest=True, fail_safe=True)
