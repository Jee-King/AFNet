...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.align_fusion = backbones.ALIGN_FUSION()


    def forward(self, train_imgs, test_imgs, train_events, test_events, train_bb, test_proposals, *args, **kwargs):
        ...
        train_events_feat = self.extract_backbone_features(train_events.reshape(-1, *train_events.shape[-3:]))
        test_events_feat = self.extract_backbone_features(test_events.reshape(-1, *test_events.shape[-3:]))

        train_low, train_high = self.align_fusion(train_feat['layer3'], train_feat['layer2'], train_events_feat['layer3'],train_events_feat['layer2'])
        test_low, test_high = self.align_fusion(test_feat['layer3'], test_feat['layer2'],
                                                  test_events_feat['layer3'], test_events_feat['layer2'])

        train_feat['layer2'] = train_low
        test_feat['layer2'] = test_low

        train_feat['layer3'] = train_high
        test_feat['layer3'] = test_high

        # train_feat['layer2'] = train_feat['layer2'] + train_events_feat['layer2']
        # test_feat['layer2'] = test_feat['layer2'] + test_events_feat['layer2']
        #
        # train_feat['layer3'] = train_feat['layer3'] + train_events_feat['layer3']
        # test_feat['layer3'] = test_feat['layer3'] + test_events_feat['layer3']
