...
class KLDiMPProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['train_events'], data['train_anno'] = self.transform['joint'](image=data['train_events'], bbox=data['train_anno'])

            data['test_images'], data['test_anno'] = self.transform['joint'](image=data['test_images'], bbox=data['test_anno'], new_roll=False)
            data['test_events'], data['test_anno'] = self.transform['joint'](image=data['test_events'], bbox=data['test_anno'], new_roll=False)
        ...
        for s in ['train', 'test']:
            ...
            crops_event, boxes = prutils.target_image_crop(data[s + '_events'], jittered_anno, data[s + '_anno'],
                                                                 self.search_area_factor, self.output_sz,
                                                                 mode=self.crop_type,
                                                                 max_scale_change=self.max_scale_change)

            crops, _ = prutils.target_image_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                 self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                 max_scale_change=self.max_scale_change)

            data[s + '_events'], data[s + '_anno'] = self.transform[s](image=crops_event, bbox=boxes, joint=False)
