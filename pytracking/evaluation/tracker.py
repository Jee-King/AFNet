...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, netname: str=None, display_name: str = None):
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        """
        params = self.get_parameters(self.netepoch)
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Initialize
        event = self._read_image(seq.frames[0])
        # image = event
        seq_name, idx = seq.frames[0].rsplit('/', 1)
        seq_name = seq_name.replace('stack240', 'img')
        idx = int(idx.split('.')[0])
        if '20fps' in seq_name:
            beishu = 12
        elif '15fps' in seq_name:
            beishu = 16
        elif '10fps' in seq_name:
            beishu = 24
        else:
            beishu = 6
        gray_id = (idx - 1) // beishu + 1
        img_path = os.path.join(seq_name, '{:05}.jpg'.format(gray_id))
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            event = self._read_image(frame_path)
            # image = event
            seq_name, idx = frame_path.rsplit('/', 1)
            seq_name = seq_name.replace('stack240', 'img')
            idx = int(idx.split('.')[0])
            if '20fps' in seq_name:
                beishu = 12
