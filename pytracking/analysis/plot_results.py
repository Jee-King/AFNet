...
def print_results_1(trackers, dataset, report_name, merge_results=False,
                  plot_types=('success'), **kwargs):
    """ Print the results for the given trackers in a formatted table
    args:
        trackers - List of trackers to evaluate
    ...
    for index in range(len(tracker_names)):
        print(tracker_names[index])
        scores = {}
        ...
        if 'success' in plot_types:
            ...
            # Index out valid sequences
            auc_curve, auc = get_auc_curve_1(ave_success_rate_plot_overlap, valid_sequence)
            scores['AUC'] = auc[:,index]
            scores['OP50'] = auc_curve[:,index, threshold_set_overlap == 0.50].mean(-1)
            scores['OP75'] = auc_curve[:,index, threshold_set_overlap == 0.75].mean(-1)
        ...
        sequence_names = [s + ' (ID={})'.format(i) for i, (s, v) in
                          enumerate(zip(sequence_names, valid_sequence.tolist())) if v]
        report_text = generate_formatted_report(sequence_names, scores, table_name=report_name)
        print(report_text)
...
def print_per_sequence_results(trackers, dataset, report_name, merge_results=False,
    ...
    avg_success_all = torch.tensor(eval_data['avg_success_all']) * 100.0
    ave_success_rate_plot_center = torch.tensor(eval_data['ave_success_rate_plot_center'])
    ...
    avg_success_all = avg_success_all[valid_sequence, :]
    ...
    success_per_tracker = {k: avg_success_all[:, i] for i, k in enumerate(tracker_disp_names)}
    report_text_success = generate_formatted_report(sequence_names, success_per_tracker)
    print(report_text)
    print('*'*30+'        Success Data')
    print(report_text_success)
