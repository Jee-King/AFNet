...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, events, info: dict) -> dict:
        ...
        init_backbone_feat = self.generate_init_samples(im, event)
    ...
    def track(self, image, events, info: dict = None) -> dict:
        self.debug_info = {}
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, event, self.get_centered_sample_pos(),
    ...
    def extract_backbone_features(self, im: torch.Tensor, event, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        event_patch, patch_coords = sample_patch_multiscale(event, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone(im_patches)
            event_feat = self.net.extract_backbone(event_patch)
            train_low, train_high = self.net.align_fusion(backbone_feat['layer3'], backbone_feat['layer2'],
                                                      event_feat['layer3'], event_feat['layer2'])
    ...
    def generate_init_samples(self, im: torch.Tensor, event) -> TensorList:
        ...
        event_patches = sample_patch_transformed(event, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms)

        # Extract initial backbone features
        with torch.no_grad():
            init_backbone_feat = self.net.extract_backbone(im_patches)
            event_init_feat = self.net.extract_backbone(event_patches)
            train_low_init, train_high_init = self.net.align_fusion(init_backbone_feat['layer3'], init_backbone_feat['layer2'],
                                                          event_init_feat['layer3'], event_init_feat['layer2'])
